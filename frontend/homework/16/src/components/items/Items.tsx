        ...
        <ul style={cssStyles.ul}>
          {filteredItems.map((item) => (
            <div style={cssStyles.todoItem} key={item.id}>
              ...
              <label className='todo-item' style={{ textDecoration: item.isStriked ? 'line-through' : 'none' ,display:'flex',alignItems:'center'}}>
                <input className='input-div-text'
                  type="checkbox"
                  checked={item.isStriked}
                  onChange={() => toggleItemStrike(item.id)}
                />
                {item.isStriked?( 
                  <div className='input-div-text'>{item.todo}</div>
                ):(
                  <input
                  data-testid='cy-input-todo'
